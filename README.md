...
[リポジトリのルート (itp2it.github.io)]
 ...
 │       ├── deploy.yml             # [★1] 自動デプロイのメインワークフロー定義ファイル（GitHub Pages用）
 │       │
 │       └── (build-and-push-docker-image.yml)
 │                                  # ◇ Dockerイメージビルド＆プッシュ用の定義ファイル (GitHub Packages用)
 │
 ├── (dist/)                        # [★4] CI/CDプロセスで使われる一時ディレクトリ
 │  　　　　　　　　　　　　　　　　　　　　　(処理の開始時に生成、終了時に消去される)
 │                                    　src/ から処理後、または、直接コピーされたファイル/フォルダが一時的に格納され、
 │ 　　　　　　　　　　　　　　　　　　　　　　その内容が Github Pages として生成される
 │
 ├── html-minifier.json             # [★3] HTML Minifyツールの設定ファイル
 ├── package.json                   # [★2] プロジェクトのメタデータと依存関係を定義するファイル
 │
 │                                  ### スクリプトの定義
 │                                  # [2-1] "clean"       : dist フォルダの中を空にする。 ※念のための処理※ 
 │                                  # [2-2] "minify-html" : src フォルダ内のすべてのHTMLファイルを最適化（minify）し、distフォルダに同じ階層構造で出力する。 "[★3]を使用します"
 │                                  # [2-3] "minify-css"  : src フォルダ内のすべてのCSS ファイルを最適化（minify）し、distフォルダに同じ階層構造で出力する。
 │                                  # [2-4] "copy-assets" : htmlとcss以外のファイル（画像やJavaScriptなど）を、bakファイルを除外しながらsrcフォルダからdistフォルダへコピーする。
 │                                     ↓↓↓
 │                                  ### スクリプトの実行
 │                                  # <2-5> "build"       : npm run コマンドで、<2-1>～<2-4>を順番に実行する。 
 │
...
### GitHub Actions の簡単な流れ
GitHub Actions を使った今回のワークフローは、以下の3つのステップで成り立っています。
...
通常、このワークフローは、あなたがローカルのPCでファイルを更新し、GitHubリポジトリにプッシュするだけで自動的に起動します。
これが、いわゆるCI/CD（継続的インテグレーション・継続的デプロイ）の基本的な考え方です。

手動で起動したい場合は、以下の手順で実行できます。
1. GitHubリポジトリの「**Actions**」タブに移動します。

左側のサイドバーで、ワークフロー名（例: ``Deploy to GitHub Pages``）をクリックします。

画面右上の「**Run workflow**」ボタンをクリックします。

<img src="workflow-01.png" width="75%">

---


#### 2. 処理の実行
**起動したら、どのファイルを使ってどんな処理をするか？**

ワークフローが起動すると、以下の**2つの重要なファイル**を使って処理が進みます。

1. ``.github/workflows/deploy.yml:`` [★1]
このファイルが、GitHub Actions に「何をすべきか」を指示する設計図です。  

**役割:**
   - Node.js環境をセットアップする。
   - ``package.json`` [★2]  に書かれたコマンド（``npm run build``）を実行する。
   - ビルド成果物をGitHub Pagesにアップロードする。
   - GitHub Pagesをデプロイする。

2. ``package.json:``
このファイルは、``npm run build`` コマンドが「具体的に何をすべきか」を定義しています。

**役割:**
 - ビルド（``npm run build``）というコマンドを実行すると、以下の3つの処理を順番に行うように指示します。
   - **minify-html**: ``src`` フォルダ内のすべてのHTMLファイルを最適化（minify）し (※[★3]を使用)、``dist``フォルダ[★4] に同じ階層で出力する。
   - **minify-css**: ``src`` フォルダ内のすべてのCSSファイルを最適化（minify）し、``dist``フォルダ[★4] に同じ階層で出力する。
   - **copy-assets**: ``html``と``css``以外のファイル（画像やJavaScriptなど）を、``bak``ファイルを除外しながら``src``フォルダから``dist``フォルダ[★4] へ**コピー**する。
...
すべての処理が成功すると、以下のことが達成されます。

- GitHub Actionsのログに**緑色のチェックマーク**✅が表示され、処理が正常に完了したことを示します。

- ``dist``フォルダの中身（最適化されたHTML/CSS、コピーされた画像など）がGitHub Pagesに公開され、ウェブサイトとして閲覧可能になります。

---

このように、複雑に見える設定も、実際は「``deploy.yml`` が ``package.json`` を呼び出し、``package.json`` が各処理を順番に実行する」というシンプルな連携で動いています。
